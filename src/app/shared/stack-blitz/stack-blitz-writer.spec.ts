import {EXAMPLE_COMPONENTS, ExampleData, LiveExample} from '@ajf/ajf-examples';
...
describe('StackBlitzWriter', () => {
  ...
  beforeEach(() => {
    ...
    // Fake the example in the `EXAMPLE_COMPONENTS`. The stack blitz writer
    // relies on module information for the example in order to read the example
    // sources from disk.
    EXAMPLE_COMPONENTS[testExampleId] = {
      module: {importSpecifier: 'cdk/my-comp'}
    } as LiveExample;
  });
...
<!-- Copyright 2020 Google LLC. All Rights Reserved.
  ...
  it('should create form element', () => {
    expect(stackBlitzWriter._createFormElement('index.ts').outerHTML)
        .toBe(
  ...
  it('should add files to form input', () => {
    ...
    stackBlitzWriter._addFileToForm(
        form, data, 'NoContent', 'test.ts', 'path/to/file', false);
    stackBlitzWriter._addFileToForm(
        form, data, 'Test', 'test.html', 'path/to/file', false);
    stackBlitzWriter._addFileToForm(
        form, data, 'Detail', 'src/detail.ts', 'path/to/file', false);

    expect(form.elements.length).toBe(3);
    expect(form.elements[0].getAttribute('name'))
        .toBe('files[src/app/test.ts]');
    expect(form.elements[1].getAttribute('name'))
        .toBe('files[src/app/test.html]');
    expect(form.elements[2].getAttribute('name'))
        .toBe('files[src/app/src/detail.ts]');
  });

  it('should open a new window with stackblitz url', fakeAsync(() => {
       let form: HTMLFormElement;
       stackBlitzWriter.constructStackBlitzForm(testExampleId, data, false)
           .then(result => {
             form = result;
             flushMicrotasks();
             ...
             expect(form.elements[6].getAttribute('name'))
                 .toBe('files[src/index.html]');
             expect(form.elements[7].getAttribute('name'))
                 .toBe('files[src/styles.scss]');
             expect(form.elements[8].getAttribute('name'))
                 .toBe('files[src/polyfills.ts]');
             expect(form.elements[9].getAttribute('name'))
                 .toBe('files[src/main.ts]');
             expect(form.elements[10].getAttribute('name'))
                 .toBe('files[src/app/material-module.ts]');
             expect(form.elements[11].getAttribute('name'))
                 .toBe('files[src/app/test.ts]');
             expect(form.elements[12].getAttribute('name'))
                 .toBe('files[src/app/test.html]');
             expect(form.elements[13].getAttribute('name'))
                 .toBe('files[src/app/src/detail.ts]');
           });
     }));
});

import {ExampleData} from '@ajf/ajf-examples';
...
export class StackBlitzButton {
  /**
   * The button becomes disabled if the user hovers over the button before the
   * StackBlitz form is created. After the form is created, the button becomes
   * enabled again. The form creation usually happens extremely quickly, but we
   * handle the case of the StackBlitz not yet being ready for people with poor
   * network connections or slow devices.
   */
  isDisabled = false;
    ...
    if (example) {
      this.stackBlitzWriter
          .constructStackBlitzForm(
              example, this.exampleData, example.includes('harness'))
  ...
  openStackBlitz(): void {
    // When the form is submitted, it must be in the document body. The standard
    // of forms is not to submit if it is detached from the document. See the
    // following chromium commit for more details:

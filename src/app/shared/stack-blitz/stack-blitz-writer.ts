import {EXAMPLE_COMPONENTS, ExampleData} from '@ajf/ajf-examples';
...
const COPYRIGHT = `Copyright (C) Gnucoop soc. coop.
...
/**
 * Path that refers to the docs-content from the "@ajf/ajf-examples"
 * package. The structure is defined in the Material repository, but we include
 * the docs-content as assets in in the CLI configuration.
 */
const DOCS_CONTENT_PATH = '/docs-content/examples-source';

const TEMPLATE_PATH = '/assets/stack-blitz/';
const TEMPLATE_FILES = [
  '.editorconfig', '.gitignore', 'angular.json', '.browserslistrc',
  'package.json', 'tsconfig.json', 'tsconfig.app.json', 'tsconfig.spec.json',
  'tslint.json', 'src/index.html', 'src/styles.scss', 'src/polyfills.ts',
  'src/main.ts', 'src/app/material-module.ts'
];
...
const dependencies = {
  '@angular/cdk': ajfVersion,
  '@angular/animations': angularVersion,
  '@angular/common': angularVersion,
  '@angular/compiler': angularVersion,
  '@angular/core': angularVersion,
  '@angular/forms': angularVersion,
  '@angular/material': ajfVersion,
  '@angular/material-moment-adapter': ajfVersion,
...
export class StackBlitzWriter {
  ...
  /**
   * Returns an HTMLFormElement that will open a new StackBlitz template with
   * the example data when called with submit().
   */
  constructStackBlitzForm(
      exampleId: string, data: ExampleData,
    ...
    const baseExamplePath = `${DOCS_CONTENT_PATH}/${
        liveExample.module.importSpecifier}/${exampleId}/`;
    ...
    return new Promise(resolve => {
      const templateContents =
          (isTest ? TEST_TEMPLATE_FILES : TEMPLATE_FILES)
              .map(
                  file => this._readFile(
                      form, data, file,
                      isTest ? TEST_TEMPLATE_PATH : TEMPLATE_PATH, isTest));

      const exampleContents = data.exampleFiles.map(
          file => this._readFile(form, data, file, baseExamplePath, isTest));

      // TODO(josephperrott): Prevent including assets to be manually checked.
      if (data.selectorName === 'icon-svg-example') {
        this._readFile(
            form, data, 'assets/img/examples/thumbup-icon.svg', '', isTest,
            false);
      }
   ...
   */
  _readFile(
      form: HTMLFormElement, data: ExampleData, filename: string, path: string,
      isTest: boolean, prependApp = true): void {
    this._http.get(path + filename, {responseType: 'text'})
        .subscribe(
            response => this._addFileToForm(
                form, data, response, filename, path, isTest, prependApp),
   ...
   */
  _addFileToForm(
      form: HTMLFormElement, data: ExampleData, content: string,
      filename: string, path: string, isTest: boolean, prependApp = true) {
   ...
   * "<material-docs-example>" and "AjfDocsExample".
   * This will replace those placeholders with the names from the example
   * metadata, e.g. "<basic-button-example>" and "BasicButtonExample"
   */
  _replaceExamplePlaceholderNames(
      data: ExampleData, fileName: string, fileContent: string): string {
    if (fileName === 'src/index.html') {
      // Replace the component selector in `index,html`.
      // For example, <material-docs-example></material-docs-example> will be
      // replaced as <button-demo></button-demo>
      fileContent =
          fileContent.replace(/material-docs-example/g, data.selectorName)
      ...
      // Replace `import {AjfDocsExample} from 'material-docs-example'`
      // will be replaced as `import {ButtonDemo} from './button-demo'`
      fileContent =
          fileContent.replace(/{AjfDocsExample}/g, `{${joinedComponentNames}}`);

      // Replace `declarations: [AjfDocsExample]`
      // will be replaced as `declarations: [ButtonDemo]`
      fileContent = fileContent.replace(
          /declarations: \[AjfDocsExample]/g,
          `declarations: [${joinedComponentNames}]`);

      // Replace `entryComponents: [AjfDocsExample]`
      // will be replaced as `entryComponents: [DialogContent]`
      fileContent = fileContent.replace(
          /entryComponents: \[AjfDocsExample]/g,
          `entryComponents: [${joinedComponentNames}]`);

      // Replace `bootstrap: [AjfDocsExample]`
      // will be replaced as `bootstrap: [ButtonDemo]`
      // This assumes the first component listed in the main component
      fileContent = fileContent.replace(
          /bootstrap: \[AjfDocsExample]/g,
          `bootstrap: [${data.componentNames[0]}]`);

      const dotIndex = data.indexFilename.lastIndexOf('.');
      const importFileName =
          data.indexFilename.slice(0, dotIndex === -1 ? undefined : dotIndex);
      fileContent =
          fileContent.replace(/material-docs-example/g, importFileName);
    }
    return fileContent;
  }

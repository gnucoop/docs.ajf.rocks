import {EXAMPLE_COMPONENTS, LiveExample} from '@ajf/ajf-examples';
import {Clipboard} from '@angular/cdk/clipboard';
import {Component, Input, NgModuleFactory, OnInit, QueryList, Type, ViewChildren, ɵNgModuleFactory} from '@angular/core';
...
export class ExampleViewer implements OnInit {
  ...
  /**
   * Map of example files that should be displayed in the view-source tab in
   * order.
   */
  ...
  selectCorrectTab() {
    ...
    for (let i = 0; i < exampleTabNames.length; i++) {
      ...
      if (tabName.toLowerCase() === extension ||
  ...
  copySource(text: string) {
    if (this.clipboard.copy(text)) {
      ...
      this.snackbar.open(
          'Copy failed. Please try again!', '', {duration: 2500});
    }
  }
  ...
  generateUrl(file: string): string {
    ...
    return this.exampleData ? `/docs-content/examples-highlighted/${
                                  this.exampleData.packagePath}/${fileName}` :
                              '';
  }
  ...
  private async _loadExampleComponent() {
    ...
    // Webpack needs to be able to statically determine possible imports for
    // proper chunk generation. Explicitly specifying the path to the `fesm2015`
    // folder as first segment instructs Webpack to generate chunks for each
    // example flat esm2015 bundle. To avoid generating unnecessary chunks for
    // source maps (which would never be loaded), we instruct Webpack to exclude
    // source map files. More details:
    // https://webpack.js.org/api/module-methods/#magic-comments.
    const moduleExports: any = await import(
        /* webpackExclude: /\.map$/ */
        '@ajf/ajf-examples/fesm2015/' + module.importSpecifier);
    this._exampleComponentType = moduleExports[componentName];
    // The components examples package is built with Ivy. This means that no
    // factory files are generated. To retrieve the factory of the AOT compiled
    // module, we simply pass the module class symbol to Ivy's module factory
    // constructor. There is no equivalent for View Engine, where factories are
    // stored in separate files. Hence the API is currently Ivy-only.
    this._exampleModuleFactory =
        new ɵNgModuleFactory(moduleExports[module.name]);
  }
  ...
  private _generateExampleTabs() {
    ...
    if (this.exampleData) {
      // Name of the default example files. If files with such name exist within
      // the example, we provide a shorthand for them within the example tabs
      // (for less verbose tabs).
      const exampleBaseFileName = `${this.example}-example`;
      const docsContentPath =
          `/docs-content/examples-highlighted/${this.exampleData.packagePath}`;


      for (const fileName of this.exampleData.files) {
        // Since the additional files refer to the original file name, we need
        // to transform the file name to match the highlighted HTML file that
        // displays the source.
        const fileSourceName =
            fileName.replace(fileExtensionRegex, '$1-$2.html');
